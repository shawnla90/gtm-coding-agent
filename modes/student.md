...
Week one this day is longer, because you are pointing the engine before you can read it. Copy `starters/reddit-buyer-signals/` into `signals/`, then edit `config/subreddits.txt` and `config/keywords.txt` for the market you want to work in rather than a market you want to sell to. Run `bash run.sh --offline` first to see the shape with no key and no cost. When you go live, the key comes from the environment: `export RAPIDAPI_KEY="..."`, read via `os.environ`, and the generated `.gitignore` already keeps `data/` out of the commit. Chapter 18 (`chapters/18-reddit-buyer-signals.md`) explains the recency and relevance guardrails, and they apply to you unchanged.
